...
func format_color(col: Color) -> String:
...
func format_variable(value: Variant) -> Variant:
    ...
    match(typeof(value)):
        ...
        TYPE_COLOR:       formatted_val = format_color(value)
        TYPE_STRING:      formatted_val = value
        TYPE_STRING_NAME: formatted_val = value as String
        TYPE_ARRAY:       formatted_val = format_array(value)
        TYPE_PACKED_VECTOR2_ARRAY: formatted_val = format_array(value as Array)
        TYPE_PACKED_STRING_ARRAY:  formatted_val = format_array(value as Array)
        _:                formatted_val = var_to_str(value)
        
    return formatted_val
    
# --- Deserialization of Data ---
func deserialize_variable(value: Variant) -> Variant:
...
func deserialize_dictionary(dict: Dictionary) -> Variant: 
...
func deserialize_string(str: String) -> Variant:
    ## Check whether this string is actually another formatted type..
    
    # Check for color
    if Color.html_is_valid(str):
        return Color.html(str)
    
    ## Normal deserialization
    return str
    
func deserialize_array(arr: Array) -> Array:
...
func _formulate_creature_data(creature: BaseCreature, desc: String = "Standard custom creature.") -> Dictionary:
    ...
    return format_dictionary(creature_data)
