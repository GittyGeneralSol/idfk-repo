...
@export var outer_width: float = 3
@export var inner_width: float = 20
@export var distance_from_bounds_inner: float = 3
@onready var self_pos: Vector2

func _ready() -> void:
    
    # Perform this logic ONLY when running the game.
    if Engine.is_editor_hint():
        return # Skip this logic in the editor
...
func _draw() -> void:
    ...
    draw_wall_lines_master(outer_width, distance_from_bounds_inner + inner_width, outer_col, outer_col, outer_col)
    draw_wall_lines_master(outer_width, distance_from_bounds_inner + inner_width + outer_width, Color.BLACK, Color.BLACK, Color.BLACK)
