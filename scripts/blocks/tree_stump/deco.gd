...
@export var circumrad: float = 115
@export var shape_color: Color = Color.SANDY_BROWN
...
func _draw():
    ...
    for ring_number in num_of_rings:
        var subtract_color: Color = Color(0.045, 0.045, 0.045, 0) * ring_number
        var subtract_cirrad: float = 35 * ring_number
        var col = shape_color - subtract_color
        var n = num_points if ring_number % 2 == 0 else 6
