...
# Modified get_parentblock(). This may wait for self to be added in tree.
func get_parent_block(obj: Node) -> Node:
    ...
    if is_instance_valid(scene_owner) and scene_owner is Block:
        return scene_owner
    else:
...
func get_property_by_path(path: String, caller: Node = null) -> Variant:
    ...
    var root_node = get_parent_block(self)
    if not is_instance_valid(root_node):
        printerr("Path failed: Could not find creature root from part '", self.name, "'.")
        return null
        
    # First, check if the given path leads to a part or not:
    var possible_part = get_part_by_name(path)
    if is_instance_valid(possible_part):
